...
public class CoreDBManager {
    ...
    public synchronized void initCoreDB(String coreDBPath) {
        if(coredb == null) {
            File file = new File(coreDBPath).getParentFile();
            if (!file.exists()) {
                file.mkdirs();
            }
            coredb = SQLiteDatabase.openOrCreateDatabase(coreDBPath, null);
        }
    }
